...
windowPlus.restore = function (defaultUrl, opts) {
  ...
  if ( _windowsProfile ) {
    ...
    sizeAndPosition = _windowsProfile.get('sizeAndPosition');
...
windowPlus.manage = function (win, uuid, userdata) {
  ...
  win.on('closed', () => {
    ...
    if ( _mainwin === win ) {
      _mainwin = BrowserWindow.getFocusedWindow() || null;
...
/**
 * @method reset
 * Return the size and position of the focused window
 * or if there are no focused windows, return the stored sizeAndPosition variable
 */
windowPlus.getSizeAndPosition = function () {

  const focusedWin = BrowserWindow.getFocusedWindow();
  if (focusedWin) {
    return {
      width: focusedWin.getSize()[0],
      height: focusedWin.getSize()[1],
      x: focusedWin.getPosition()[0],
      y: focusedWin.getPosition()[1]
    };
  }
...
function _saveWindowStates () {
  ...
  _windowsProfile.set('sizeAndPosition', sizeAndPosition);
